...
<head>
    ...
    <?php
        ...
        $ct=$_GET['ct'];
        $knsp1=mysqli_query($con,"select * from hinhanh inner join sanpham on hinhanh.MaSP=sanpham.MaSP where LoaiSP='$ct'");
        $rows=mysqli_num_rows($knsp1);
        }
        if(isset($_GET['mt'])&& $_GET['mt']){
            $ct=$_GET['mt'];
            $knsp1=mysqli_query($con,"select * from hinhanh inner join sanpham on hinhanh.MaSP=sanpham.MaSP where $ct=1");
            $rows=mysqli_num_rows($knsp1);
        }
        if(isset($_GET['sr'])&& $_GET['sr']){
            $ct=$_GET['sr'];
            $knsp1=mysqli_query($con,"select * from hinhanh inner join sanpham on hinhanh.MaSP=sanpham.MaSP inner join noisx on sanpham.mansx=noisx.mansx inner join nhasx on sanpham.manhasx = nhasx.manhasx where LoaiSP='$ct' or TenSP='$ct' or DienGiai='$ct' or TenNSX='$ct'");
            $rows=mysqli_num_rows($knsp1);
        }
    ...
    <div class="khungbreadcrumb">
        <div class="breadcrumb-tree">
            <ul>
                ...
                <li class="active"><?php if($rows==0){echo 'Search'; }else{ echo $ct;}?></li>
            </ul>
        </div>
    </div>
    <div class="sp">
        <div class="khungsp">
            <div class="tieudesp">
                <h3><a href="#"><?php if($rows==0){echo 'Search';}else{ echo $ct; }?></a></h3>
            </div>
            <div class="listsp">
                <div class="danhsachct">
                <?php if($rows>0){ while ($sp1=mysqli_fetch_array($knsp1)) {
												?>
                    <div class="sanpham">
